...
import { DEFAULT_CODE as SHARED_DEFAULT_CODE, EXECUTABLE_LANGUAGE_IDS, LANGUAGE_BY_ID } from "../constants/languages";
...
export default function CodingRoom() {
  ...
  useEffect(() => {
    ...
    socket.on("load-code", ({ code, language }) => {
      const finalCode = code || SHARED_DEFAULT_CODE[language] || "// Write your code here";
  ...
  }, [roomId, currentUser, navigate, password]);
  ...
  const handleRun = useCallback(async () => {
    if (!EXECUTABLE_LANGUAGE_IDS.includes(langRef.current)) {
      const label = LANGUAGE_BY_ID[langRef.current]?.label || langRef.current;
      setShowOutput(true);
      setOutput({
        success: false,
        status: "Unsupported",
        output: `${label} is available for syntax highlighting, but code execution is not configured for it.`,
      });
      toast.error("Execution is not available for this language", {
        description: `${label} is syntax highlighting only.`,
      });
      return;
    }
    ...
    <div className="flex flex-col h-screen w-screen bg-cyber-900 overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 py-2 bg-cyber-800 border-b border-cyber-700 shrink-0 z-20">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden sm:flex items-center gap-2 px-3 py-1 bg-cyber-900 border border-cyber-600 rounded-lg min-w-0">
          ...
          <button
            onClick={copyInviteLink}
            className="sm:hidden flex items-center gap-1.5 px-3 py-1 bg-cyber-900 border border-cyber-600 rounded-lg text-xs text-gray-300 hover:border-neon-purple"
            title={`Copy invite link: ${inviteLink}`}
          >
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          >
            <CodeEditor
              ...
              defaultCodes={SHARED_DEFAULT_CODE}
